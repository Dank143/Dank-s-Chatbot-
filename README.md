...
| **L**atency | How quickly the model answers. Combines time-to-first-token and tokens-per-second with NIM serving responsiveness. A model that is usually fast but occasionally stalls scores lower than one that is steadily quick. Flash and smaller models lead here; massive frontier models trade latency for depth. |
